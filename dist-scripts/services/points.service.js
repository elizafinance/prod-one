...
export class PointsService {
     ...
     */
    async addPoints(walletAddress, pointsDelta, options) {
        var _a;
        if (!walletAddress)
            throw new Error('Wallet address is required.');
        if (typeof pointsDelta !== 'number' || isNaN(pointsDelta))
            throw new Error('pointsDelta must be a valid number.');
        const { reason, metadata = {}, emitEvent = true, allowNegativeTotal = false, actionType, dbSession } = options;
        const usersCollection = this.db.collection('users');
        const actionsCollection = this.db.collection('actions');
        const squadsCollection = this.db.collection('squads');
        // 1. Fetch the user
        ...
        const userUpdate = { $set: { points: newPointsTotal, updatedAt: new Date() } };
        if (pointsDelta > 0 && actionType && !((_a = user.completedActions) === null || _a === void 0 ? void 0 : _a.includes(actionType))) {
        ...
        if (user.squadId && pointsDelta !== 0) { // Update if points change, positive or negative
            const squadUpdateResult = await squadsCollection.updateOne({ squadId: user.squadId }, {
                $inc: { totalSquadPoints: pointsDelta },
                $set: { updatedAt: new Date() }
            }, { session: dbSession });
            if (squadUpdateResult.modifiedCount > 0 && emitEvent) {
                try {
                    await rabbitmqService.publishToExchange(rabbitmqConfig.eventsExchange, rabbitmqConfig.routingKeys.squadPointsUpdated, {
        ...
        if (emitEvent) {
            try {
                await rabbitmqService.publishToExchange(rabbitmqConfig.eventsExchange, rabbitmqConfig.routingKeys.userPointsUpdated, // Ensure this routing key is defined in rabbitmqConfig
                {
                    walletAddress,
        ...
        return Object.assign(Object.assign({}, user), { points: newPointsTotal, completedActions: userUpdate.$addToSet ? [...(user.completedActions || []), actionType] : user.completedActions });
    }
    /**
     ...
     * @param options Options for awarding points.
     */
    async setPoints(walletAddress, absolutePoints, options) {
        if (!walletAddress)
            throw new Error('Wallet address is required.');
     ...
     */
    async convertPointsToAirNft(walletAddress, requestedTierId, options) {
        if (!walletAddress)
            throw new Error('Wallet address is required.');
        const usersCollection = this.db.collection('users');
        // 1. Find the requested tier configuration
        const tierConfig = AIR_NFT_TIERS.find(t => t.tier === requestedTierId);
        if (!tierConfig) {
            return { success: false, message: `Invalid AIR NFT tier ID: ${requestedTierId}` };
        }
        // 2. Fetch user's current AIR points
        const user = await usersCollection.findOne({ walletAddress }, { session: options === null || options === void 0 ? void 0 : options.dbSession });
        if (!user) {
        ...
        await this.addPoints(walletAddress, -pointsToDeduct, {
            ...
            dbSession: options === null || options === void 0 ? void 0 : options.dbSession,
        });
        ...
        if (rabbitmqConfig.routingKeys.airNftMinted) { // Check if routing key is defined
            try {
                await rabbitmqService.publishToExchange(rabbitmqConfig.eventsExchange, rabbitmqConfig.routingKeys.airNftMinted, {
                    walletAddress,
