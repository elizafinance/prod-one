...
export default function CreateSquadPage() {
    ...
    const [tierRequirements, setTierRequirements] = useState(null);
    ...
    return (<main className="flex flex-col items-center min-h-screen p-4 sm:p-8 bg-white text-gray-900">
      <div className="w-full max-w-md mx-auto my-10">
        ...
        {pointsLoading && (<div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded text-blue-700 text-center flex flex-col items-center">
        ...
        {!pointsLoading && !canCreate && (<div className="p-4 bg-red-50 border border-red-200 rounded-lg text-center my-4">
            <p className="text-red-700">
              ...
              {tierRequirements?.tiers.map(tier => (<span key={tier.tier} className="text-sm text-gray-600">
        ...
        <form onSubmit={handleSubmit} className="bg-white border border-gray-200 shadow-xl rounded-xl p-6 sm:p-8 space-y-6">
          <div>
            ...
            <input type="text" id="squadName" value={squadName} onChange={(e) => setSquadName(e.target.value)} className="w-full p-3 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none text-gray-900 placeholder-gray-400 disabled:bg-gray-200" placeholder="The Legends" maxLength={30} required disabled={!canCreate || pointsLoading}/>
          </div>
          ...
          <div>
            ...
            <textarea id="description" value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="w-full p-3 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none text-gray-900 placeholder-gray-400 disabled:bg-gray-200" placeholder="A brief description of your squad's mission..." maxLength={150} disabled={!canCreate || pointsLoading}/>
          </div>

          {error && <p className="text-sm text-red-600 text-center">{error}</p>}

          <button type="submit" disabled={isLoading || !connected || !canCreate || pointsLoading} className="w-full py-3 px-5 bg-green-500 hover:bg-green-600 disabled:bg-gray-400 text-white font-semibold rounded-lg shadow-md hover:shadow-lg transform hover:scale-105 transition-all duration-150 ease-in-out disabled:opacity-70 disabled:cursor-not-allowed">
            {isLoading ? (<span className="flex items-center justify-center">
                ...
                Creating Squad...
              </span>) : pointsLoading ? 'Checking...' : 'Create Squad'}
          </button>
        </form>
