...
export async function POST(request) {
    ...
    try {
        ...
        if (squad.leaderWalletAddress !== invitingUserWalletAddress) {
            return NextResponse.json({ error: 'Only the squad leader can send invitations.' }, { status: 403 });
        }
        ...
        await createNotification(db, targetUserWallet, 'squad_invite_received', `You have been invited to join the squad "${squad.name}" by @${invitingUserXUsername}.`, squadId, squad.name, invitingUserWalletAddress, invitingUserXUsername, invitationId);
        return NextResponse.json({
