...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (squad.memberWalletAddresses.length >= memberLimit) {
            ...
            await squadJoinRequestsCollection.updateOne({ requestId: requestId }, { $set: { status: 'rejected', updatedAt: new Date() } });
        ...
        const userUpdateResult = await usersCollection.updateOne({ walletAddress: joinRequest.requestingUserWalletAddress }, { $set: { squadId: squad.squadId, updatedAt: new Date() } });
        ...
        const squadUpdateResult = await squadsCollection.updateOne({ squadId: squad.squadId }, {
        ...
        await squadJoinRequestsCollection.updateOne({ requestId: requestId }, { $set: { status: 'approved', updatedAt: new Date() } });
        // 8. Send notifications
        // To requester
        await createNotification(db, joinRequest.requestingUserWalletAddress, 'squad_join_request_approved', `Your request to join squad "${squad.name}" was approved! Welcome aboard!`, squad.squadId, squad.name, leaderWalletAddress, session.user.xUsername || undefined);
        ...
        if (updatedSquad) {
            for (const memberAddr of updatedSquad.memberWalletAddresses) {
                if (memberAddr !== joinRequest.requestingUserWalletAddress) {
                    await createNotification(db, memberAddr, 'squad_member_joined', `@${joinRequest.requestingUserXUsername || joinRequest.requestingUserWalletAddress.substring(0, 6)} has joined your squad "${updatedSquad.name}"!`, updatedSquad.squadId, updatedSquad.name, joinRequest.requestingUserWalletAddress, joinRequest.requestingUserXUsername || undefined);
                }
            }
        }
