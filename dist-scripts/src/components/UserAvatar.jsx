...
const UserAvatar = ({ profileImageUrl, username, size = 'md', className = '', }) => {
    ...
    return (<div className={`${sizeClass} relative rounded-full overflow-hidden flex items-center justify-center bg-gray-200 ${className}`}>
      {imageUrl ? (
        // Using next/image with unoptimized to avoid issues with Twitter's image server
        <Image src={imageUrl} alt={`${username || 'User'}'s avatar`} fill unoptimized className="object-cover" onError={() => setImageError(true)}/>) : (<span className="text-gray-700 font-bold">
          {fallbackText}
        </span>)}
    </div>);
};
