...
export default function UserProfilePage() {
    ...
    if (isLoading)
        return <main className="flex flex-col items-center justify-center min-h-screen bg-white text-gray-700"><p>Loading Profile...</p></main>;
    if (error)
        return <main className="flex flex-col items-center justify-center min-h-screen bg-white text-red-700"><p>Error: {error}</p><button onClick={() => router.back()} className="mt-2 px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">Go Back</button></main>;
    if (!profileData)
        return <main className="flex flex-col items-center justify-center min-h-screen bg-white text-gray-700"><p>Profile not available.</p></main>;
    ...
    return (<main className="flex flex-col items-center min-h-screen p-4 sm:p-8 bg-white text-gray-900">
      <div className="w-full max-w-2xl mx-auto my-10 bg-white border border-gray-200 shadow-xl rounded-xl p-6 sm:p-10">
        ...
        <div className="flex flex-col items-center text-center mb-8">
          <UserAvatar profileImageUrl={profileData.xProfileImageUrl} username={profileData.xUsername} size="lg" className="mb-4"/>
        ...
        {isOwnProfile && (<div className="my-6">
        ...
        <div className="space-y-6">
          ...
          {/* Airdrop Tier */}
          {profileData.highestAirdropTierLabel && (<div className="p-6 bg-gray-50 border border-gray-200 rounded-lg shadow-md">
          ...
          {/* Squad Info */}
           {profileData.squadInfo && (<div className="p-6 bg-gray-50 border border-gray-200 rounded-lg shadow-md">
          ...
          {/* Badges */}
          {profileData.earnedBadgeIds && profileData.earnedBadgeIds.length > 0 && (<div className="p-6 bg-gray-50 border border-gray-200 rounded-lg shadow-md">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                return badge ? (badge.isSpecial ? (<GlowingBadge key={badgeId} icon={badge.icon} label={badge.label} color={badge.color} glowColor={badge.glowColor || "rgba(255, 255, 255, 0.5)"} size="md"/>) : (<span key={badgeId} className={`px-3 py-1 text-xs font-semibold rounded-full ${badge.color}`}>
                        {badge.icon} {badge.label}
                      </span>)) : (<span key={badgeId} className="px-3 py-1 text-xs font-semibold rounded-full bg-gray-200 text-gray-700">
          ...
          {/* Referred By Info */}
          {profileData.referredBy && (<div className="p-6 bg-gray-50 border border-gray-200 rounded-lg shadow-md">
              <h2 className="text-sm font-semibold text-purple-700 uppercase tracking-wider mb-2">Referred By</h2>
              <div className="flex items-center gap-3">
                <UserAvatar profileImageUrl={profileData.referredBy.xProfileImageUrl} username={profileData.referredBy.xUsername} size="sm"/>
                <div>
                  {profileData.referredBy.xUsername ? (<span className="text-md text-gray-800">@{profileData.referredBy.xUsername}</span>) : (<span className="text-md text-gray-800 font-mono">{profileData.referredBy.walletAddress}</span>)}
                </div>
              </div>
            </div>)}
        </div>
        ...
        <div className="mt-10 text-center">
          <div className="flex flex-wrap justify-center gap-3">
            <ShareProfileButton walletAddress={walletAddress} username={profileData.xUsername} points={profileData.points} airdropTier={profileData.highestAirdropTierLabel}/>
