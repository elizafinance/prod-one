...
export default function AllNotificationsPage() {
    ...
    const fetchAllNotifications = useCallback(async (markAsReadOnClick = false) => {
        if (!connected)
            return; // Don't fetch if wallet not connected (session won't be there for API)
        setIsLoading(true);
        setError(null);
        try {
    ...
    return (<main className="flex flex-col items-center min-h-screen p-4 sm:p-8 bg-gradient-to-b from-gray-900 to-gray-800 text-white">
      <div className="w-full max-w-3xl mx-auto py-8 sm:py-12">
        <div className="flex justify-between items-center mb-10">
          ...
          <Link href="/" passHref>
            <button className="bg-gray-600 hover:bg-gray-700 text-white font-semibold py-2 px-5 rounded-lg shadow-md hover:shadow-lg transform hover:scale-105 transition-all duration-150 ease-in-out whitespace-nowrap">
              Back to Dashboard
            </button>
          </Link>
        </div>
        ...
        <div className="mb-6 flex justify-end">
            {notifications.length > 0 && unreadCount > 0 && (<button onClick={handleMarkAllAsRead} className="text-sm bg-blue-500 hover:bg-blue-600 text-white font-semibold py-1.5 px-3 rounded-md shadow-sm transition-colors">
        ...
        {!isLoading && !error && notifications.length === 0 && (<div className="text-center py-10 bg-gray-800 bg-opacity-50 p-6 rounded-lg shadow-xl">
        ...
        {!isLoading && !error && notifications.length > 0 && (<div className="bg-white/5 backdrop-blur-sm shadow-2xl rounded-xl p-2 sm:p-4">
            <ul className="divide-y divide-gray-700/50">
              {notifications.map(notif => (<NotificationItem key={notif._id} notification={notif} onMarkAsRead={handleMarkOneAsRead}/> // Use _id for key
            ))}
            </ul>
          </div>)}
      </div>
