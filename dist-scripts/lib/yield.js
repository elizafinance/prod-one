...
export function getYieldProgram(connection, wallet) {
...
export async function findPoolAddress() {
    const [poolAddress] = await PublicKey.findProgramAddress([Buffer.from('pool'), AIR_TOKEN_MINT.toBuffer()], YIELD_PROGRAM_ID);
    return poolAddress;
}
// Find the token vault address
export async function findTokenVaultAddress() {
    const [vaultAddress] = await PublicKey.findProgramAddress([Buffer.from('vault'), AIR_TOKEN_MINT.toBuffer()], YIELD_PROGRAM_ID);
    return vaultAddress;
}
// Find a user's stake account
export async function findStakeAccount(walletAddress) {
    const poolAddress = await findPoolAddress();
    const [stakeAccount] = await PublicKey.findProgramAddress([Buffer.from('stake'), poolAddress.toBuffer(), walletAddress.toBuffer()], YIELD_PROGRAM_ID);
    return stakeAccount;
}
// Stake tokens
export async function stakeTokens(connection, wallet, amount, lockPeriod) {
    ...
    const stakerTokenAccount = await getAssociatedTokenAddress(AIR_TOKEN_MINT, wallet.publicKey);
    // Create transaction
...
// Unstake tokens
export async function unstakeTokens(connection, wallet) {
    const program = getYieldProgram(connection, wallet);
    const poolAddress = await findPoolAddress();
    const tokenVault = await findTokenVaultAddress();
    const stakeAccount = await findStakeAccount(wallet.publicKey);
...
// Claim rewards
export async function claimRewards(connection, wallet) {
    const program = getYieldProgram(connection, wallet);
    const poolAddress = await findPoolAddress();
    const tokenVault = await findTokenVaultAddress();
    const stakeAccount = await findStakeAccount(wallet.publicKey);
...
export async function getStakeInfo(connection, walletAddress) {
    try {
        const dummyWallet = {
        ...
        const stakeInfo = await program.account.stakeAccount.fetchNullable(stakeAccount);
        return stakeInfo;
    }
    catch (error) {
...
export async function getPoolInfo(connection) {
    try {
        ...
        const program = getYieldProgram(connection, dummyWallet);
        const poolAddress = await findPoolAddress();
        const poolInfo = await program.account.pool.fetchNullable(poolAddress);
        return poolInfo;
    }
    catch (error) {
...
export function calculateEstimatedRewards(stakeAmount, apyPercent, daysStaked) {
...
export function formatUnlockDate(startTimestamp, lockPeriodSeconds) {
...
export function useStakingInfo(connection) {
