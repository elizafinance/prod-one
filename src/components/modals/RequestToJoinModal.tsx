...
// Assuming Textarea might not exist, will use a standard HTML textarea if shadcn one isn't found by linter.
...
interface RequestToJoinModalProps {
  ...
  squadName: string;
  squadId: string;
  onSubmit: (squadId: string, message?: string) => Promise<void>;
...
const RequestToJoinModal: React.FC<RequestToJoinModalProps> = ({ 
  ...
  let TextareaComponent: any = 'textarea'; // Default to HTML textarea
  try {
      ...
      <DialogContent className="sm:max-w-[480px] bg-white border-gray-300 shadow-xl">
        ...
        <div className="py-4 space-y-3">
          ...
          <TextareaComponent 
            ...
            placeholder={`E.g., "Hi, I'm active daily and would love to contribute to ${squadName}!"`}
            maxLength={500}
            rows={3}
            className="w-full p-2 border border-gray-300 focus:ring-blue-500 focus:border-blue-500 rounded-md disabled:opacity-70"
        ...
        <DialogFooter className="mt-4 sm:justify-end gap-2 flex-col sm:flex-row">
          ...
          <Button 
            ...
            disabled={isSubmitting}
            className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white disabled:opacity-70"
          >
