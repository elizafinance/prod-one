...
export default function LeaderboardPage() {
    ...
    const renderBadges = (badges) => {
        ...
        {badges.map(badgeId => {
                ...
                if (!badge)
                    return null;
                return badge.isSpecial ? (<GlowingBadge key={badgeId} icon={badge.icon} label={badge.label} color={badge.color} glowColor={badge.glowColor || "rgba(255, 255, 255, 0.5)"} size="sm"/>) : (<span key={badgeId} className={`px-2 py-0.5 text-xs font-semibold rounded-full ${badge.color}`}>
              {badge.icon}
            </span>);
            })}
      </div>);
    ...
    return (<main className="flex flex-col items-center min-h-screen p-4 sm:p-8 bg-white text-gray-900">
      <div className="w-full max-w-5xl mx-auto py-8 sm:py-12">
        <div className="flex justify-between items-center mb-10">
          ...
          <Link href="/" passHref>
            <button className="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-5 rounded-lg shadow-md hover:shadow-lg transform hover:scale-105 transition-all duration-150 ease-in-out whitespace-nowrap">
        ...
        {!isLoading && !error && leaderboard.length === 0 && (<div className="text-center py-10 bg-gray-100 p-6 rounded-lg shadow-lg border border-gray-200">
        ...
        {!isLoading && !error && leaderboard.length > 0 && (<div className="overflow-x-auto shadow-xl rounded-xl border border-gray-200 bg-white">
            <table className="min-w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                ...
                return (<tr key={entry.walletAddress + index + rank} className={rowClasses}>
                      ...
                      <td className="py-4 px-4 sm:px-6 align-middle">
                        <div className="flex items-center gap-3">
                          <UserAvatar profileImageUrl={entry.xProfileImageUrl} username={entry.xUsername} size="sm"/>
                          <div>
                            {entry.xUsername ? (<Link href={`/profile/${entry.walletAddress}`} passHref>
                                <span className="text-gray-800 hover:text-blue-600 cursor-pointer hover:underline font-medium">@{entry.xUsername}</span>
                              </Link>) : (<Link href={`/profile/${entry.walletAddress}`} passHref>
