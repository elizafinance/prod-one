...
const TokenBalance = () => {
    ...
    useEffect(() => {
        ...
        fetchUserTokenAccount()
            ...
            .catch((e) => {
            if (e.message.includes("The account of type [Token] was not found at the provided address")) {
                updateTokenAccount(null);
            }
        });
    }, [umiSigner, updateTokenAccount]);
    return (<div className="flex gap-4 items-center">
      <img src={TokenImage.src} className="aspect-square w-8 h-8 rounded-full" alt="token image"/>
      {tokenAccount || tokenAccount === null ? (<div className="w-full text-center">
          {tokenAccount === null
                ? "0"
                : "Balance: " + formatTokenAmount(tokenAccount.amount, 6)}
        </div>) : (<Skeleton className="w-full min-w-[150px] h-8"/>)}
    </div>);
};
