...
export async function POST(request) {
    ...
    try {
        ...
        if (user.squadId) {
            ...
            if (currentSquad) {
                ...
                if (currentSquad.leaderWalletAddress === currentUserWalletAddress) {
                    await invitationsCollection.updateOne({ invitationId }, { $set: { status: 'declined', updatedAt: new Date(), notes: 'User is a squad leader' } });
                ...
                for (const memberAddr of currentSquad.memberWalletAddresses) {
                    if (memberAddr !== currentUserWalletAddress) {
                        await createNotification(db, memberAddr, 'squad_member_left', `@${currentUserXUsername} has left your squad "${currentSquad.name}" to join another squad.`, currentSquad.squadId, currentSquad.name, currentUserWalletAddress, currentUserXUsername);
                    }
                }
            }
        }
        ...
        await squadsCollection.updateOne({ squadId: invitation.squadId }, {
            $addToSet: { memberWalletAddresses: currentUserWalletAddress },
            $inc: { totalSquadPoints: pointsToContribute },
            $set: { updatedAt: new Date() },
        });
        await usersCollection.updateOne({ walletAddress: currentUserWalletAddress }, {
        ...
        await invitationsCollection.updateOne({ invitationId }, { $set: { status: 'accepted', updatedAt: new Date() } });
        // Create notification for the user who sent the invite (or squad leader)
        await createNotification(db, invitation.invitedByUserWalletAddress, 'squad_invite_accepted', `@${currentUserXUsername} has accepted your invitation to join "${squadToJoin.name}"!`, squadToJoin.squadId, squadToJoin.name, currentUserWalletAddress, currentUserXUsername);
        ...
        squadToJoin.memberWalletAddresses.forEach(async (memberAddress) => {
            if (memberAddress !== currentUserWalletAddress && memberAddress !== invitation.invitedByUserWalletAddress) {
                await createNotification(db, memberAddress, 'squad_member_joined', `@${currentUserXUsername} has joined your squad "${squadToJoin.name}"!`, squadToJoin.squadId, squadToJoin.name, currentUserWalletAddress, currentUserXUsername);
            }
        });
