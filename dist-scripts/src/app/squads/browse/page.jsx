...
export default function BrowseSquadsPage() {
    ...
    const [mySquadInfo, setMySquadInfo] = useState(null);
    ...
    const [selectedSquadForRequest, setSelectedSquadForRequest] = useState(null);
    const [isSubmittingRequest, setIsSubmittingRequest] = useState(false);
    // New state for user's pending join requests
    const [currentUserPendingRequests, setCurrentUserPendingRequests] = useState([]);
    const fetchSquadsAndUserData = useCallback(async () => {
        ...
        try {
            ...
            setError(err.message || 'Could not load squads data.');
            console.error(err);
        }
        setIsLoading(false);
    }, [connected, publicKey]);
    ...
    const handleOpenRequestModal = (squad) => {
        if (!connected || !publicKey) {
    ...
    const handleSubmitJoinRequest = async (squadIdToRequest, message) => {
        if (!publicKey)
            return;
    ...
    return (<main className="flex flex-col items-center min-h-screen p-4 sm:p-8 bg-white text-gray-900">
      <div className="w-full max-w-4xl mx-auto py-8 sm:py-12">
        ...
        {isLoading && (<div className="text-center py-10"><p className="text-xl text-gray-600">Searching for Squads...</p><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-600 mx-auto mt-4"></div></div>)}
        {error && <p className="text-center text-red-700 bg-red-100 p-4 rounded-lg border border-red-300">Error: {error}</p>}
        
        {!isLoading && !error && squads.length === 0 && (<div className="text-center py-10 bg-gray-100 p-6 rounded-lg shadow-lg border border-gray-200">
            <p className="text-2xl text-gray-700 mb-3">No squads found. Why not start your own?</p>
          </div>)}

        {!isLoading && !error && squads.length > 0 && (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {squads.map((squad) => {
                ...
                return (<div key={squad.squadId} className="bg-white border border-gray-200 shadow-lg rounded-lg p-6 flex flex-col justify-between min-h-[280px]">
                  <div>
                  ...
                  <div className="mt-4 space-y-2">
                    {(!connected || !publicKey) ? (<p className="text-xs text-center text-orange-600 py-2">Connect wallet to interact</p>) : mySquadInfo?.squadId ? (<button disabled className="w-full py-2 px-4 bg-gray-300 text-gray-600 font-semibold rounded-lg cursor-not-allowed">
                        Already in a Squad
                      </button>) : isSquadFull ? (<button disabled className="w-full py-2 px-4 bg-red-200 text-red-600 font-semibold rounded-lg cursor-not-allowed">
                        Squad Full
                      </button>) : hasPendingRequestForThisSquad ? (<button disabled className="w-full py-2 px-4 bg-yellow-200 text-yellow-700 font-semibold rounded-lg cursor-not-allowed">
                        Request Pending
                      </button>) : (<button onClick={() => handleOpenRequestModal(squad)} className="w-full py-2 px-4 bg-sky-500 hover:bg-sky-600 text-white font-semibold rounded-lg shadow hover:shadow-md transition-colors">
                        Request to Join
                      </button>)}
      ...
      {selectedSquadForRequest && (<RequestToJoinModal isOpen={isRequestModalOpen} onClose={() => setIsRequestModalOpen(false)} squadName={selectedSquadForRequest.name} squadId={selectedSquadForRequest.squadId} onSubmit={handleSubmitJoinRequest} isSubmitting={isSubmittingRequest}/>)}
    </main>);
}
