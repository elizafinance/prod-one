...
export default function NotificationItem({ notification, onMarkAsRead }) {
    ...
    const handleAcceptInvite = async (e) => {
        e.stopPropagation();
        if (!isSquadInvite || !notification.squadId)
            return;
    ...
    return (<li className={`p-3 border-b border-gray-700 last:border-b-0 hover:bg-gray-700/30 transition-colors ${notification.isRead ? 'opacity-75' : 'bg-gray-700/20'}`} onClick={isSquadInvite ? undefined : handleNotificationClick} // Only allow general click for non-squad invites
    >
      <div className="flex items-start">
        ...
        <div className="flex-grow">
          ...
          {isSquadInvite && !notification.isRead && (<div className="mt-2 flex space-x-2">
              <button className="px-3 py-1 bg-green-600 hover:bg-green-700 text-white text-xs rounded-md transition-colors disabled:opacity-50" onClick={handleAcceptInvite} disabled={isProcessing !== null}>
                {isProcessing === 'accept' ? 'Accepting...' : 'Accept'}
              </button>
              <button className="px-3 py-1 bg-red-600 hover:bg-red-700 text-white text-xs rounded-md transition-colors disabled:opacity-50" onClick={handleDeclineInvite} disabled={isProcessing !== null}>
