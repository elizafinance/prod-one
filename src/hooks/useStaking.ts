...
export const useStaking = () => {
    ...
    try {
      ...
      const initPoolIx = await program.methods
        .initPool(
          ...
          isLinear,
          new BN(baseValue),
          new BN(slopeOrExponent),
          new BN(minDepositAmount),
          new BN(maxDepositAmount)
        )
        .accounts({
          authority,
          poolAuthority,
  ...
  const initStakeEntry = useCallback(async (
    poolState: PublicKey,
    position: PublicKey,
    ...
    try {
      ...
      const [userStakeEntry] = PublicKey.findProgramAddressSync(
  ...
  const stake = useCallback(async (
    ...
    try {
      ...
      if (!stakeEntryExists) {
        ...
        const positionAccount = await getPositionAddress(positionMint.toString() as Address);
  ...
  const increasePoolRewards = useCallback(async (
    poolState: PublicKey,
    amount: number,
    autoUnpause: boolean | null = null,
    maxMultiplier: number | null = null
  ) => {
    if (!program || !provider || !provider.wallet.publicKey) {
      throw new Error('Wallet not connected');
    }
    ...
    try {
      ...
      const authorityRewardTokenAccount = getAssociatedTokenAddressSync(
        rewardTokenMint,
        authority,
        false,
        TOKEN_2022_PROGRAM_ID
      ...
      const increasePoolRewardsIx = await program.methods
        .increasePoolRewards(
          new BN(amount),
          autoUnpause !== null ? autoUnpause : null,
          maxMultiplier !== null ? new BN(maxMultiplier) : null,
        )
        .accounts({
          authority,
          elizaConfig,
  ...
  const updatePoolConfig = useCallback(async (
    poolState: PublicKey,
    params: {
      rewardDuration?: number;
      minDepositAmount?: number;
      maxDepositAmount?: number;
      isCurveLinear?: boolean;
      curveBase?: number;
      curveSlopeOrExponent?: number;
    ...
    try {
      ...
      const updatePoolConfigIx = await program.methods
        .updatePoolConfig(
          ...
          minDepositAmount,
          maxDepositAmount,
          params.isCurveLinear !== undefined ? params.isCurveLinear : null,
          curveBase,
          curveSlopeOrExponent,
          params.newFeeCollector !== undefined ? params.newFeeCollector : null,
          params.newAuthority !== undefined ? params.newAuthority : null,
  ...
  const updateElizaConfig = useCallback(async (
    params: {
      rewardsAuthority?: PublicKey;
      positionHarvestAuthority?: PublicKey;
      pauseAllPools?: number;
    }
  ) => {
    if (!program || !provider || !provider.wallet.publicKey) {
      throw new Error('Wallet not connected');
    ...
    try {
      ...
      const updateElizaConfigIx = await program.methods
        .updateElizaConfig(
          params.rewardsAuthority !== undefined ? params.rewardsAuthority : null,
          params.positionHarvestAuthority !== undefined ? params.positionHarvestAuthority : null,
          params.pauseAllPools !== undefined ? params.pauseAllPools : null
        )
        .accounts({
          authority,
          elizaConfig,
  ...
  const harvestPosition = useCallback(async (
    poolState: PublicKey,
    position: PublicKey,
    positionMint: PublicKey,
    stakeEntry: PublicKey,
