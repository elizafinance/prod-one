...
async function distributeTokens(proposal) {
...
async function processEndedProposals() {
    ...
    try {
        ...
        for (const proposal of activeProposalsToEnd) {
            console.log(`Processing proposal: ${proposal._id.toString()} (${proposal.tokenName})`);
            ...
            let currentProposalStatus = 'closed_failed'; // Default to failed
            if (netVoteWeight > CRON_PROPOSAL_PASS_THRESHOLD) {
                currentProposalStatus = 'closed_passed';
                console.log(`PROPOSAL PASSED (pre-execution): Proposal ${proposal._id.toString()} (${proposal.tokenName}) for squad ${proposal.squadName}. Net weight: ${netVoteWeight}.`);
                ...
                console.log(`PROPOSAL FAILED: Proposal ${proposal._id.toString()} (${proposal.tokenName}) for squad ${proposal.squadName}. Net weight: ${netVoteWeight}.`);
...
async function archiveOldClosedProposals() {
    ...
    try {
        if (mongoose.connection.readyState !== 1) {
            ...
            if (!process.env.MONGODB_URI)
                throw new Error('MONGODB_URI required for archiving.');
            dbConnection = await mongoose.connect(process.env.MONGODB_URI);
