...
export default function NotificationsPanel({ isOpen, onClose, onUpdateUnreadCount }) {
    const [notifications, setNotifications] = useState([]); // Use UnifiedNotification
    ...
    const fetchNotifications = useCallback(async (markAsReadOnOpen = false) => {
        ...
        try {
            ...
            if (markAsReadOnOpen && data.notifications && data.notifications.length > 0) {
                ...
                const unreadIds = data.notifications.filter((n) => !n.isRead).map((n) => n._id);
    ...
    if (!isOpen)
        return null;
    return (<div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40" onClick={onClose} // Close if clicking outside the panel
    >
      <div className="fixed top-16 right-4 sm:right-8 w-full max-w-md bg-gray-800 shadow-2xl rounded-lg border border-gray-700 z-50 max-h-[70vh] flex flex-col" onClick={(e) => e.stopPropagation()} // Prevent close when clicking inside panel
    >
        <div className="flex justify-between items-center p-4 border-b border-gray-700">
          <h2 className="text-lg font-semibold text-white">Notifications</h2>
          {notifications.some(n => !n.isRead) && (<button onClick={handleMarkAllAsRead} className="text-xs text-blue-400 hover:underline">Mark all as read</button>)}
          <button onClick={onClose} className="text-gray-400 hover:text-white">&times;</button>
        </div>

        <div className="overflow-y-auto flex-grow">
          {isLoading && (<div className="p-8 flex justify-center items-center">
              <div className="animate-spin h-8 w-8 border-t-2 border-b-2 border-blue-500 rounded-full"></div>
              <p className="ml-3 text-gray-400">Loading notifications...</p>
            </div>)}
          
          {error && (<div className="p-4 text-center text-red-400 bg-red-900/20 m-4 rounded-lg">
              ...
              {showErrorDetails && (<p className="mt-2 text-xs text-left bg-black/30 p-2 rounded overflow-auto">
                  If you are seeing authentication errors, try refreshing the page or reconnecting your wallet.
                </p>)}
            </div>)}
          
          {!isLoading && !error && notifications.length === 0 && (<p className="p-8 text-center text-gray-400">You have no notifications yet.</p>)}

          {!isLoading && !error && notifications.length > 0 && (<ul>
              {notifications.map(notif => (<NotificationItem key={notif._id} notification={notif} onMarkAsRead={handleMarkOneAsRead}/>))}
            </ul>)}
        </div>
