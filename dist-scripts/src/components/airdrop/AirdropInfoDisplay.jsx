...
const AirdropInfoDisplay = ({ onNotConnected, showTitle = true, onTotalAirdropChange }) => {
    ...
    const [initialAirdropAllocation, setInitialAirdropAllocation] = useState(null);
    ...
    useEffect(() => {
        if (onTotalAirdropChange) {
            const currentPointsShare = userPoints !== null && userPoints > 0 && totalCommunityPoints !== null && totalCommunityPoints > 0
                ? (userPoints / totalCommunityPoints) * airdropPoolSize
                : 0;
            const currentTotalEstimatedAirdrop = (initialAirdropAllocation || 0) +
    ...
    const fetchAirdropData = useCallback(async () => {
        ...
        let fetchedUserPoints = null;
        let fetchedTotalCommunityPoints = null;
        let fetchError = null;
        try {
    ...
    if (isLoading) {
        return (<div className="w-full max-w-lg p-6 my-8 mx-auto bg-white border border-gray-200 rounded-xl shadow-lg text-center">
    ...
    if (error) {
        return (<div className="w-full max-w-lg p-6 my-8 mx-auto bg-red-50 border border-red-200 rounded-xl shadow-lg text-center flex flex-col items-center">
    ...
    const pointsShare = userPoints !== null && userPoints > 0 && totalCommunityPoints !== null && totalCommunityPoints > 0
        ? (userPoints / totalCommunityPoints) * airdropPoolSize
        : 0;
    ...
    return (<div className={`w-full max-w-lg p-6 md:p-8 my-8 mx-auto ${backgroundColor} ${borderColor} rounded-2xl shadow-xl font-sans`}>
      {showTitle && (<div className="flex items-center justify-center mb-6">
      ...
      <div className="space-y-5">
        ...
        <div className={`p-5 ${backgroundColor} rounded-lg ${borderColor} border shadow-md`}>
          ...
          {totalCommunityPoints === null && userPoints !== null && userPoints > 0 && (<p className={`text-xs text-center ${textColor} opacity-75`}>(Finalizing estimate based on total community points...)</p>)}
