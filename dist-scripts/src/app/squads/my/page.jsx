...
export default function MySquadPage() {
    ...
    const fetchUserPoints = useCallback(async (walletAddress) => {
        if (!walletAddress || isLoadingPoints)
            return;
    ...
    const fetchMySquadData = useCallback(async (userWalletAddress) => {
        ...
        if (!userWalletAddress || isFetchingSquad || userCheckedNoSquad || (mySquadData && hasLoadedData))
            return;
    ...
    const getMaxMembersForPoints = (points) => {
        if (!points || !tierRequirements?.tiers)
            return 'Loading...';
    ...
    return (<main className="flex flex-col items-center min-h-screen p-4 sm:p-8 bg-white text-gray-900">
      <div className="w-full max-w-3xl">
        ...
        <div className="w-full p-5 bg-indigo-50 border border-indigo-200 rounded-xl shadow-md mb-4">
          ...
          {!isFetchingSquad && mySquadData && (<div className="text-center space-y-4">
              <div className="p-4 bg-white/80 rounded-lg border border-gray-200">
                ...
                <div className="mt-3 text-xs bg-indigo-100 p-2 rounded border border-indigo-200">
                  {isUserLeader ? (<p className="text-indigo-700 font-medium">You are the leader of this squad!</p>) : (<p className="text-indigo-700">Leader: <span className="font-mono">{mySquadData.leaderWalletAddress.substring(0, 6)}...{mySquadData.leaderWalletAddress.substring(mySquadData.leaderWalletAddress.length - 4)}</span></p>)}
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                {!isUserLeader && (<button className="py-2 px-4 bg-red-500 hover:bg-red-600 text-white text-xs font-semibold rounded-lg transition-colors w-full shadow hover:shadow-md">
                    Leave Squad
                  </button>)}
              </div>
            </div>)}
          
          {!isFetchingSquad && !mySquadData && !error && (<div className="text-center">
              ...
              {isLoadingPoints ? (<div className="flex justify-center items-center py-3">
                  ...
                  <div className="mt-4 text-center">
                    {canCreateSquad ? (<div className="mb-4 p-3 bg-green-100 rounded-lg border border-green-300">
                      ...
                      </div>) : (<div className="mb-4 p-3 bg-gray-100 rounded-lg border border-gray-300">
                  ...
                  {userCheckedNoSquad && (<button onClick={handleForceRefresh} className="py-2 px-4 bg-blue-500 hover:bg-blue-600 text-white text-sm font-semibold rounded-lg transition-colors shadow hover:shadow-md mt-3">
                      Refresh Squad Data
                    </button>)}
                </>)}
            </div>)}
