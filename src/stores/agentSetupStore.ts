...
export type AgentSetupMode = 'DEFAULT' | 'CUSTOM';
...
interface AgentSetupState {
  ...
  sharePercent: number;
  tosAccepted: boolean;
  currentStep: number; // 1: Choice, 2: Name (custom only), 3: Share, 4: TOS, 5: Deploy
  isModalOpen: boolean;
}

interface AgentSetupActions {
  setMode: (mode: AgentSetupMode) => void;
  setName: (name: string) => void;
  setSharePercent: (percent: number) => void;
...
const initialState: AgentSetupState = {
  mode: 'DEFAULT',
...
export const useAgentSetupStore = create<AgentSetupState & AgentSetupActions>()((set, get) => ({
  ...initialState,
  setMode: (mode) => set(() => {
    if (mode === 'DEFAULT') {
  ...
  setSharePercent: (percent) => set({ sharePercent: percent }),
  ...
  openModal: () => set({ ...initialState, isModalOpen: true, name: 'DeFAIZA', mode: 'DEFAULT' }),
  closeModal: () => set({ isModalOpen: false }),
  reset: () => set((state) => {
    const tempInitialState = { ...initialState };
    delete (tempInitialState as Partial<AgentSetupState>).isModalOpen;
    return { ...tempInitialState, isModalOpen: state.isModalOpen, name: 'DeFAIZA', mode: 'DEFAULT' };
  }),
})); 