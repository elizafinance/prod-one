...
const baseHandler = withAuth(async (request, session) => {
    try {
        ...
        let newEarnedBadgeIds = userToUpdate.earnedBadgeIds || [];
        let userHighestAirdropTierLabel = userToUpdate.highestAirdropTierLabel;
        ...
        if (referrerCodeFromQuery && !userToUpdate.referredBy && userToUpdate.walletAddress !== newSolanaWalletAddress /* to avoid self-referral if wallet was xid initially */) {
            ...
            if (referrer) {
                ...
                if (referrer.walletAddress && referrer.walletAddress !== newSolanaWalletAddress) {
                    ...
                    const updateResult = await usersCollection.updateOne({ walletAddress: referrer.walletAddress }, referrerUpdate);
                    ...
                    if (referrer.squadId) {
                        ...
                        const squadUpdateResult = await squadsCollection.updateOne({ squadId: referrer.squadId }, { $inc: { totalSquadPoints: pointsToAwardReferrer }, $set: { updatedAt: new Date() } } // Use pointsToAwardReferrer which includes boost
                        );
        ...
        let determinedHighestTierLabelThisSession = undefined;
        if (airdropAmount > 0) {
        ...
        const userUpdateResult = await usersCollection.updateOne({ xUserId: xUserId }, // Find by xUserId
        { $set: updateData });
        ...
        if (potentiallyUpdatedUser?.squadId && pointsGainedThisSession > 0) {
            await squadsCollection.updateOne({ squadId: potentiallyUpdatedUser.squadId }, {
