...
export async function POST(request) {
    ...
    try {
        ...
        if (!squad) {
            return NextResponse.json({ error: 'Associated squad not found.' }, { status: 404 });
        }
        ...
        const result = await invitationsCollection.updateOne({ invitationId, status: 'pending' }, { $set: { status: 'revoked', updatedAt: new Date() } });
        if (result.matchedCount === 0) {
            return NextResponse.json({ error: 'Invitation could not be revoked (it might have been processed already).' }, { status: 409 });
        }
        await createNotification(db, invitation.invitedUserWalletAddress, 'squad_invite_revoked', `Your invitation to join "${invitation.squadName}" was revoked by @${revokingUserXUsername}.`, invitation.squadId, invitation.squadName, revokingUserWalletAddress, revokingUserXUsername);
