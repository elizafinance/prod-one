...
function getSquadTierInfo(points) {
    if (points >= TIER_3_POINTS)
        return { tier: 3, maxMembers: TIER_3_MAX_MEMBERS };
    if (points >= TIER_2_POINTS)
        return { tier: 2, maxMembers: TIER_2_MAX_MEMBERS };
    if (points >= TIER_1_POINTS)
        return { tier: 1, maxMembers: TIER_1_MAX_MEMBERS };
    return { tier: 0, maxMembers: 0 }; // Not eligible
}
// Function to check for profanity (simple case-insensitive check)
function containsProfanity(name, list) {
...
export async function POST(request) {
    ...
    try { // Moved try block to encompass body parsing and subsequent logic
        ...
        await usersCollection.updateOne({ walletAddress: leaderWalletAddress }, {
            $set: {
                squadId: newSquadId,
                updatedAt: new Date()
            }
