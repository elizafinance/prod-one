...
export default function SquadDetailsPage() {
    ...
    const [isKickingMember, setIsKickingMember] = useState(null); // Store walletAddress of member being kicked
    const [inviteType, setInviteType] = useState('wallet');
    ...
    const [foundTwitterUser, setFoundTwitterUser] = useState(null);
    const [sentPendingInvites, setSentPendingInvites] = useState([]);
    const [isFetchingSentInvites, setIsFetchingSentInvites] = useState(false);
    const [isRevokingInvite, setIsRevokingInvite] = useState(null); // invitationId being revoked
    // State for managing join requests (for leader)
    const [joinRequests, setJoinRequests] = useState([]);
    const [isFetchingJoinRequests, setIsFetchingJoinRequests] = useState(false);
    const [isProcessingJoinRequest, setIsProcessingJoinRequest] = useState(null); // requestId being processed
    ...
    const fetchSentPendingInvitesForSquad = useCallback(async (currentSquadId) => {
        if (!currentSquadId || !connected)
            return;
        ...
        try {
            ...
            if (response.ok) {
                ...
                const squadSpecificInvites = (data.invitations || []).filter((inv) => inv.squadId === currentSquadId && inv.status === 'pending');
    ...
    const fetchJoinRequestsForSquad = useCallback(async (currentSquadId) => {
        if (!currentSquadId || !connected)
            return;
    ...
    const fetchSquadDetails = useCallback(async () => {
        ...
        try {
            ...
            setError(err.message || 'Could not load squad details.');
            setHasLoadedInitialData(true);
        }
        setIsLoading(false);
    ...
    useEffect(() => {
        const fetchMyPendingRequests = async () => {
            ...
            try {
                ...
                if (response.ok) {
                    const data = await response.json();
                    const hasPending = (data.requests || []).some((req) => req.squadId === squadId && req.status === 'pending');
    ...
    const handleEditSquadSubmit = async (e) => {
        e.preventDefault();
        if (!squadDetails)
            return;
        ...
        try {
            const payload = {};
            if (editableSquadName !== squadDetails.name)
                payload.squadName = editableSquadName;
            if (editableDescription !== squadDetails.description)
            ...
            if (response.ok) {
                ...
                if (data.squad)
                    setSquadDetails(data.squad); // Update local state with new squad data
                setIsEditingSquad(false);
            }
            else {
    ...
    const handleKickMember = async (memberWalletAddressToKick) => {
    ...
    const handleTransferLeadership = async (newLeaderWalletAddress) => {
        if (!squadDetails || !isUserLeader) {
    ...
    const handleRevokeInvite = async (invitationIdToRevoke) => {
    ...
    const handleSubmitJoinRequest = async (targetSquadId, message) => {
    ...
    if (isLoading)
        return <main className="flex items-center justify-center min-h-screen bg-white text-gray-700"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-purple-600"></div><p className='ml-3 text-lg'>Loading Squad Details...</p></main>;
    if (error)
        return <main className="flex flex-col items-center justify-center min-h-screen bg-white text-red-700"><p className="text-xl mb-4">Error: {error}</p><Link href="/squads/browse"><button className='p-2 bg-blue-500 hover:bg-blue-600 rounded text-white'>Back to Browse Squads</button></Link></main>;
    if (!squadDetails)
        return <main className="flex flex-col items-center justify-center min-h-screen bg-white text-gray-700"><p className="text-xl mb-4">Squad not found.</p><Link href="/squads/browse"><button className='p-2 bg-blue-500 hover:bg-blue-600 rounded text-white'>Back to Browse Squads</button></Link></main>;
    return (<main className="flex flex-col items-center min-h-screen p-4 sm:p-8 bg-white text-gray-900">
      <div className="w-full max-w-3xl mx-auto my-10 bg-white border border-gray-200 shadow-xl rounded-xl p-6 sm:p-8">
        <div className="flex justify-between items-start mb-6">
          <div>
            {!isEditingSquad ? (<h1 className="text-4xl font-bold font-spacegrotesk tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-purple-500 via-pink-600 to-red-600">
                {squadDetails?.name}
              </h1>) : (<h1 className="text-4xl font-bold font-spacegrotesk tracking-tight text-gray-800">
                Edit Squad Info
              </h1>)}
          ...
          <div className="flex flex-col space-y-2 items-end flex-shrink-0 ml-4">
            ...
            {isUserLeader && !isEditingSquad && (<button onClick={() => {
        ...
        {isEditingSquad && (<form onSubmit={handleEditSquadSubmit} className="mb-6 p-6 bg-gray-50 border border-gray-200 rounded-lg space-y-4">
            <div>
              <label htmlFor="editableSquadName" className="block text-sm font-medium text-gray-700 mb-1">Squad Name</label>
              <input type="text" id="editableSquadName" value={editableSquadName} onChange={(e) => setEditableSquadName(e.target.value)} className="w-full p-2 bg-white border border-gray-300 rounded-md text-gray-900 focus:ring-blue-500 focus:border-blue-500" maxLength={30}/>
            </div>
            <div>
              <label htmlFor="editableDescription" className="block text-sm font-medium text-gray-700 mb-1">Description</label>
              <textarea id="editableDescription" value={editableDescription} onChange={(e) => setEditableDescription(e.target.value)} rows={3} className="w-full p-2 bg-white border border-gray-300 rounded-md text-gray-900 focus:ring-blue-500 focus:border-blue-500" maxLength={150}/>
            </div>
            <div className="flex justify-end space-x-3">
              <button type="button" onClick={() => setIsEditingSquad(false)} disabled={isSavingEdit} className="py-2 px-4 bg-gray-400 hover:bg-gray-500 text-white font-semibold rounded-md disabled:opacity-50">
                Cancel
              </button>
              <button type="submit" disabled={isSavingEdit} className="py-2 px-4 bg-green-500 hover:bg-green-600 text-white font-semibold rounded-md disabled:opacity-50">
        ...
        <div className="mb-8">
          ...
          <ul className="space-y-2 max-h-72 overflow-y-auto bg-gray-50 border border-gray-200 p-3 rounded-lg">
            {squadDetails.membersFullDetails && squadDetails.membersFullDetails.length > 0 ? (squadDetails.membersFullDetails.map(member => (<li key={member.walletAddress} className="p-3 bg-white border border-gray-200 rounded text-sm text-gray-700 flex justify-between items-center hover:bg-gray-100 transition-colors">
                  <div className="flex items-center gap-3">
                    <UserAvatar profileImageUrl={member.xProfileImageUrl} username={member.xUsername} size="sm"/>
                    <div>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    {isUserLeader && member.walletAddress !== currentUserWalletAddress && (<>
                        <button onClick={() => handleTransferLeadership(member.walletAddress)} disabled={isKickingMember === member.walletAddress} className="px-2 py-1 text-xs bg-blue-500 hover:bg-blue-600 text-white rounded-md disabled:opacity-50 whitespace-nowrap" title="Transfer leadership to this member">
                          {isKickingMember === member.walletAddress ? 'Transferring...' : 'Make Leader'}
                        </button>
                      <button onClick={() => handleKickMember(member.walletAddress)} disabled={isKickingMember === member.walletAddress} className="px-2 py-1 text-xs bg-red-500 hover:bg-red-600 text-white rounded-md disabled:opacity-50">
                        {isKickingMember === member.walletAddress ? 'Kicking...' : 'Kick'}
                      </button>
                      </>)}
                  </div>
                </li>))) : (<li className="p-2 text-sm text-gray-500">No member details available or squad is empty.</li>)}
          </ul>
        </div>

        <div className="mt-8 border-t border-gray-300 pt-6">
          {connected && isUserMember && !isUserLeader && (<div className="mb-6">
              <h3 className="text-lg font-semibold text-gray-800 mb-2">Squad Actions</h3>
              <button onClick={handleLeaveSquad} disabled={isLeaving} className="w-full py-2.5 px-5 bg-red-500 hover:bg-red-600 disabled:bg-gray-400 text-white font-semibold rounded-lg shadow-md hover:shadow-lg transition-all duration-150 ease-in-out">
                {isLeaving ? 'Leaving Squad...' : 'Leave Squad'}
              </button>
            </div>)}

          {isUserLeader && squadDetails && (<div>
              ...
              <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg mb-6">
                ...
                {!isFetchingJoinRequests && joinRequests.length > 0 && (<ul className="space-y-3 max-h-72 overflow-y-auto">
                    {joinRequests.map(req => (<li key={req.requestId} className="p-3 bg-white border border-gray-200 rounded-md shadow-sm">
                        <div className="flex items-start gap-3">
                          <UserAvatar profileImageUrl={req.requestingUserXProfileImageUrl} username={req.requestingUserXUsername} size="sm"/>
                        ...
                        <div className="mt-3 flex space-x-2 justify-end">
                          <button onClick={() => handleProcessJoinRequest(req.requestId, 'reject')} disabled={isProcessingJoinRequest === req.requestId} className="px-3 py-1 text-xs bg-red-500 hover:bg-red-600 text-white rounded-md disabled:opacity-60">
                            {isProcessingJoinRequest === req.requestId ? 'Rejecting...' : 'Reject'}
                          </button>
                          <button onClick={() => handleProcessJoinRequest(req.requestId, 'approve')} disabled={isProcessingJoinRequest === req.requestId} className="px-3 py-1 text-xs bg-green-500 hover:bg-green-600 text-white rounded-md disabled:opacity-60">
              ...
              <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg mb-6">
                ...
                <div className="flex mb-4 bg-gray-200 rounded-lg p-1 w-fit">
                  <button onClick={() => setInviteType('wallet')} className={`px-3 py-1.5 text-sm font-medium rounded ${inviteType === 'wallet'
                ...
                setFoundTwitterUser(null);
            }} className={`px-3 py-1.5 text-sm font-medium rounded ${inviteType === 'twitter'
                ? 'bg-sky-500 text-white'
                : 'text-gray-600 hover:text-gray-900'}`}>
                    By Twitter
                  </button>
                ...
                {inviteType === 'wallet' ? (<div className="space-y-2">
                    <label htmlFor="inviteeWallet" className="block text-sm font-medium text-gray-700">
                      Wallet Address to Invite:
                    </label>
                    <div className="flex gap-2">
                      <input type="text" id="inviteeWallet" value={inviteeWalletAddress} onChange={(e) => setInviteeWalletAddress(e.target.value)} className="flex-grow p-2 bg-white border border-gray-300 rounded-md text-gray-900 placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500" placeholder="Enter wallet address"/>
                      <button onClick={handleSendInvite} disabled={isSendingInvite || !inviteeWalletAddress.trim()} className="py-2 px-4 bg-sky-500 hover:bg-sky-600 text-white font-semibold rounded-md shadow disabled:opacity-70">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <input type="text" id="inviteeTwitter" value={inviteeTwitterHandle} onChange={(e) => setInviteeTwitterHandle(e.target.value)} className="flex-grow p-2 bg-white border border-gray-300 rounded-md text-gray-900 placeholder-gray-400 focus:ring-blue-500 focus:border-blue-500" placeholder="Enter Twitter handle (with or without @)"/>
                        <button onClick={searchUserByTwitterHandle} disabled={isSearchingTwitterUser || !inviteeTwitterHandle.trim()} className="py-2 px-4 bg-indigo-500 hover:bg-indigo-600 text-white font-semibold rounded-md shadow disabled:opacity-70">
                          {isSearchingTwitterUser ? 'Searching...' : 'Search'}
                        </button>
                      </div>
                    </div>
                    
                    {foundTwitterUser && (<div className="p-3 bg-gray-200 rounded-lg flex items-center gap-3">
                        <UserAvatar profileImageUrl={foundTwitterUser.xProfileImageUrl} username={foundTwitterUser.xUsername} size="md"/>
                        <div className="flex-grow">
                        ...
                        <button onClick={handleSendInvite} disabled={isSendingInvite} className="py-1.5 px-3 bg-sky-500 hover:bg-sky-600 text-white font-semibold rounded-md text-sm shadow disabled:opacity-70">
                          {isSendingInvite ? 'Sending...' : 'Send Invite'}
                        </button>
                      </div>)}
                  </div>)}
              ...
              <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg mb-6">
                ...
                {!isFetchingSentInvites && sentPendingInvites.length > 0 && (<ul className="space-y-2 max-h-48 overflow-y-auto">
                    {sentPendingInvites.map(invite => (<li key={invite.invitationId} className="p-2 bg-gray-200 rounded-md text-sm flex justify-between items-center">
                        <div>
                        ...
                        <button onClick={() => handleRevokeInvite(invite.invitationId)} disabled={isRevokingInvite === invite.invitationId} className="px-2 py-1 text-xs bg-orange-500 hover:bg-orange-600 text-white rounded-md disabled:opacity-50">
              ...
              {squadDetails.leaderReferralCode && squadDetails.squadId && (<div className="p-4 bg-gray-50 border border-gray-200 rounded-lg mb-6">
                  ...
                  <div className="flex items-center justify-center bg-gray-200 p-2 rounded">
                    <input type="text" readOnly value={`https://squad.defairewards.net/?ref=${squadDetails.leaderReferralCode}&squadInvite=${squadDetails.squadId}`} className="text-gray-700 text-sm break-all bg-transparent outline-none flex-grow p-1"/>
                    <button onClick={() => handleCopyToClipboard(`https://squad.defairewards.net/?ref=${squadDetails.leaderReferralCode}&squadInvite=${squadDetails.squadId}`)} className="ml-2 py-1 px-2 text-xs bg-blue-500 text-white rounded hover:bg-blue-600 transition-colors">
                      Copy
                    </button>
                  </div>
                </div>)}
              ...
              <div className="mt-4 pt-4 border-t border-gray-300/50">
                ...
                <button onClick={handleLeaveSquad} disabled={isLeaving} className="w-full py-2.5 px-5 bg-red-600 hover:bg-red-700 disabled:bg-gray-400 text-white font-semibold rounded-lg shadow-md hover:shadow-lg transition-all duration-150 ease-in-out">
        ...
        {connected && !isUserMember && squadDetails && (<div className="mt-8 border-t border-gray-300 pt-6 text-center">
            {hasPendingRequestForThisSquad ? (<button disabled className="py-2.5 px-6 bg-yellow-200 text-yellow-700 font-semibold rounded-lg cursor-not-allowed">
                Request Pending
              </button>) : (<button onClick={handleOpenRequestModal} className="py-2.5 px-6 bg-sky-500 hover:bg-sky-600 text-white font-semibold rounded-lg shadow-md hover:shadow-lg transition-all duration-150 ease-in-out">
      ...
      {squadDetails && (<RequestToJoinModal isOpen={isRequestModalOpen} onClose={() => setIsRequestModalOpen(false)} squadName={squadDetails.name} squadId={squadDetails.squadId} onSubmit={handleSubmitJoinRequest} isSubmitting={isSubmittingJoinRequest}/>)}
    </main>);
}
