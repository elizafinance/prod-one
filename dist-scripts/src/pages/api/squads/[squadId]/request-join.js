...
export default async function handler(req, res) {
    ...
    try {
        ...
        await createNotification(db, targetSquad.leaderWalletAddress, 'squad_join_request_received', `${session.user.xUsername ? '@' + session.user.xUsername : userWalletAddress.substring(0, 6)} wants to join your squad "${targetSquad.name}"`, targetSquad.squadId, targetSquad.name, userWalletAddress, session.user.xUsername || undefined, requestId);
