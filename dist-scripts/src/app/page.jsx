...
export default function HomePage() {
    ...
    const [isProcessingInvite, setIsProcessingInvite] = useState(null); // invitationId being processed
    ...
    const [hasSufficientDefai, setHasSufficientDefai] = useState(null); // null = not checked, false = insufficient, true = sufficient
    ...
    const fetchPendingInvites = useCallback(async () => {
        if (!wallet.connected || !session)
            return; // Need session for authenticated API call
    ...
    const checkDefaiBalance = useCallback(async (userPublicKey, solanaConnection) => {
        if (!userPublicKey) {
    ...
    const activateRewardsAndFetchData = useCallback(async (connectedWalletAddress, xUserId, userDbId) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setUserData(data);
                if (data.airdropAmount !== undefined)
                    setAirdropCheckResult(data.airdropAmount);
    ...
    const logSocialAction = async (actionType) => {
    ...
    return (<main className="flex flex-col items-center min-h-screen p-4 sm:p-8 bg-white text-gray-900 font-sans relative">
      ...
      <div className="w-full max-w-3xl mx-auto pt-8 flex flex-col items-center relative">
        ...
        <div className="absolute top-[-40px] left-1/2 transform -translate-x-1/2 z-0 opacity-50 pointer-events-none">
          <img src={Illustration.src} alt="Illustration" className="w-[400px] sm:w-[500px] h-auto"/>
        </div>
        ...
        {authStatus !== "authenticated" || !isRewardsActive || !wallet.connected ? (<div className="w-full mb-8">
            <div className="relative flex items-center mt-2 mb-6">
              <input type="text" value={typedAddress} onChange={(e) => setTypedAddress(e.target.value)} placeholder="Enter any Solana address to check eligibility" className="w-full p-4 pl-5 pr-32 bg-gray-50 border border-gray-200 rounded-full focus:ring-2 focus:ring-[#2B96F1] focus:border-[#2B96F1] outline-none text-gray-800" disabled={isCheckingAirdrop || (authStatus === "authenticated" && isRewardsActive)}/>
              <button onClick={handleInitialAirdropCheck} className="absolute right-2 bg-[#2B96F1] hover:bg-blue-500 text-white font-medium py-2 px-6 rounded-full transition-all disabled:opacity-50" disabled={isCheckingAirdrop || (authStatus === "authenticated" && isRewardsActive)}>
                {isCheckingAirdrop ? 'Checking...' : 'Check Eligibility'}
              </button>
            </div>
            {airdropCheckResult !== null && (<div className="mt-6 p-5 bg-blue-50 border border-blue-200 rounded-lg text-center shadow-sm">
                  {typeof airdropCheckResult === 'number' ? (airdropCheckResult > 0 ? (<p className="text-center text-3xl font-bold mb-4 animate-pulse">
                        ...
                        </div>)) : (<p className="text-center text-lg text-red-600 mb-4">
        ...
        <div className="w-full max-w-xl mb-32"> 
          ...
          {authStatus === "authenticated" && !wallet.connected && (<div className="my-6 w-full text-center p-4 bg-gray-50 rounded-lg border border-gray-200">
          ...
          {/* Other sections */}
          {showInsufficientBalanceMessage && (<div className="w-full my-6 p-5 bg-blue-50 border border-blue-200 rounded-lg text-center shadow-md">
          ...
          {showPointsSection && userData && (<div className="w-full max-w-lg mt-2 flex flex-col items-center">
              ...
              {userData.referralCode && (<div className="my-4 p-4 bg-gray-100 rounded-lg text-center w-full">
                  <div className="flex justify-center items-center mb-2">
                    <p className="text-md font-semibold text-gray-800">Your Referral Link (Share & Earn!):</p>
                    {userData.activeReferralBoosts && userData.activeReferralBoosts.length > 0 && (<span className="ml-2 px-2 py-0.5 text-xs font-bold text-black bg-yellow-400 rounded-full animate-pulse">
                        BOOST ACTIVE!
                      </span>)}
                  </div>
              ...
              {userData.activeReferralBoosts && userData.activeReferralBoosts.length > 0 && (<div className="w-full max-w-md p-5 bg-gradient-to-br from-indigo-50 to-purple-50 border border-purple-200 rounded-xl shadow-lg mt-6 mb-4">
                  ...
                  <ul className="space-y-3">
                    {userData.activeReferralBoosts.map(boost => (<li key={boost.boostId} className="p-4 bg-white border border-gray-200 rounded-lg shadow-sm">
              ...
              {typeof userData.referralsMadeCount === 'number' && userData.referralsMadeCount > 0 && (<div className="my-3 text-center">
              ...
              {userData.walletAddress && (<div className="w-full max-w-md mt-6 mb-4 text-center">
                  <Link href={`/profile/${userData.walletAddress}`} passHref>
                    <button className="py-2.5 px-6 bg-purple-600 hover:bg-purple-700 text-white font-semibold rounded-lg shadow-md hover:shadow-lg transform hover:scale-105 transition-all duration-150 ease-in-out">
              ...
              {!mySquadData && pendingInvites.length > 0 && (<div className="w-full max-w-md p-5 bg-teal-50 border border-teal-200 rounded-xl shadow-md mt-8 mb-4">
                  ...
                  <ul className="space-y-3">
                    {pendingInvites.map(invite => (<li key={invite.invitationId} className="p-3 bg-white/80 rounded-lg shadow">
                        ...
                        <div className="flex gap-2 mt-2">
                          <button onClick={() => handleInviteAction(invite.invitationId, 'accept')} disabled={isProcessingInvite === invite.invitationId} className="flex-1 py-1.5 px-3 text-sm bg-green-500 hover:bg-green-600 text-white font-semibold rounded-md disabled:opacity-70">
                            {isProcessingInvite === invite.invitationId ? 'Processing...' : 'Accept'} 
                          </button>
                          <button onClick={() => handleInviteAction(invite.invitationId, 'decline')} disabled={isProcessingInvite === invite.invitationId} className="flex-1 py-1.5 px-3 text-sm bg-red-500 hover:bg-red-600 text-white font-semibold rounded-md disabled:opacity-70">
              ...
              <div className="mt-2 mb-4 flex flex-wrap justify-center gap-3 sm:gap-4 w-full">
                ...
                {isRewardsActive && currentTotalAirdropForSharing > 0 && (<button onClick={handleShareToX} className="w-full sm:w-auto text-white font-bold py-3 px-6 rounded-full transition-all duration-150 ease-in-out hover:scale-105 hover:shadow-lg hover:shadow-blue-500/50 whitespace-nowrap" style={{ backgroundColor: '#2563EB' }}><ShareIcon /> Flex my $AIR on X</button>)}
              ...
              {userData.points !== null && (<div className="mt-2 mb-8 w-full">
                  ...
                  <div className="bg-gray-100 p-3 sm:p-4 rounded-lg shadow">
                    ...
                    return (<li key={activity.id} className="flex justify-between items-center p-2 border-b border-gray-200 last:border-b-0">
          ...
          {authStatus === "authenticated" && wallet.connected && isCheckingDefaiBalance && (<div className="my-4 text-center text-gray-600">
