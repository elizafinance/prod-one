...
export async function POST(request, { params }) {
    ...
    try {
        ...
        if (!memberWalletAddressToKick) {
            return NextResponse.json({ error: 'Member to kick required.' }, { status: 400 });
        }
        if (leaderWalletAddress === memberWalletAddressToKick) {
            return NextResponse.json({ error: 'Leader cannot kick self.' }, { status: 400 });
        }
        ...
        if (!squad.memberWalletAddresses.includes(memberWalletAddressToKick)) {
            return NextResponse.json({ error: 'User not in squad.' }, { status: 400 });
        }
        const kickedUserDoc = await usersCollection.findOne({ walletAddress: memberWalletAddressToKick });
        if (!kickedUserDoc) {
            return NextResponse.json({ error: 'Member to kick not found in database.' }, { status: 404 });
        }
        const pointsToDeduct = kickedUserDoc.points || 0;
        const kickedUserXUsername = kickedUserDoc?.xUsername || memberWalletAddressToKick;
        await squadsCollection.updateOne({ squadId: squadIdToManage }, {
        ...
        await createNotification(db, memberWalletAddressToKick, 'squad_kicked', `You have been kicked from the squad "${squad.name}" by the leader @${leaderXUsername}.`, squad.squadId, squad.name, leaderWalletAddress, leaderXUsername);
        ...
        for (const memberAddr of remainingMembers) {
            await createNotification(db, memberAddr, 'squad_member_left', `@${kickedUserXUsername} was kicked from your squad "${squad.name}" by the leader.`, squad.squadId, squad.name, memberWalletAddressToKick, kickedUserXUsername);
        }
